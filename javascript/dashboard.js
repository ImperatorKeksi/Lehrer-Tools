...
class DashboardController {
    ...
    render() {
        ...
        this.renderWidgets();
        this.initProfileSettings(); // New: Initialize profile logic
    }

    // ========================================
    // PROFILE MANAGEMENT (Moved from Chat)
    // ========================================
    async initProfileSettings() {
        this.profileElements = {
            username: document.getElementById('settingsUsernameDB'),
            status: document.getElementById('settingsStatusDB'),
            preview: document.getElementById('profilePreviewDB'),
            uploadZone: document.getElementById('profileUploadZoneDB'),
            fileInput: document.getElementById('profileImgInputDB'),
            saveBtn: document.getElementById('saveProfileBtnDB')
        };

        if (!this.profileElements.saveBtn) return;

        // Load initial data
        await this.loadProfile();

        // Event Listeners
        this.profileElements.uploadZone.addEventListener('click', () => this.profileElements.fileInput.click());
        this.profileElements.fileInput.addEventListener('change', (e) => this.handleProfileUpload(e));
        this.profileElements.saveBtn.addEventListener('click', () => this.saveProfile());
    }

    async loadProfile() {
        try {
            const response = await fetch('../php/chat_api.php?action=current_user');
            const data = await response.json();
            if (data.success && data.data) {
                const user = data.data;
                if (this.profileElements.username) this.profileElements.username.value = user.display_name || user.username;
                if (this.profileElements.status) this.profileElements.status.value = user.custom_status || 'online';
                if (this.profileElements.preview) {
                    // Utility to fix asset paths (re-implemented here or usage of global if available)
                    const fixPath = (p) => {
                        if (!p) return '';
                        if (p.startsWith('http') || p.startsWith('data:')) return p;
                        return '../' + p.replace(/^\.\.\//, '');
                    };
                    const avatarSrc = fixPath(user.profile_img) || this.createUserAvatar(user.display_name || user.username);
                    this.profileElements.preview.src = avatarSrc;
                    this.profileElements.preview.dataset.originalPath = user.profile_img || '';
                }
            }
        } catch (e) {
    ...
    async handleProfileUpload(event) {
        ...
        try {
            this.showNotification('⏳ Bild wird hochgeladen...', 'info');
            const response = await fetch('../php/upload.php', { method: 'POST', body: formData });
            const data = await response.json();
            if (data.success) {
                const fixPath = (p) => '../' + p.replace(/^\.\.\//, '');
                this.profileElements.preview.src = fixPath(data.file_url);
                this.profileElements.preview.dataset.originalPath = data.file_url;
                this.showNotification('✅ Bild hochgeladen (Speichere zum Übernehmen)', 'success');
            }
        } catch (e) {
            this.showNotification('❌ Fehler beim Upload', 'danger');
        }
    }

    async saveProfile() {
        const username = this.profileElements.username.value.trim();
        const status = this.profileElements.status.value;
        const profileImg = this.profileElements.preview.dataset.originalPath || '';

        if (!username) {
            this.showNotification('⚠️ Bitte gib einen Namen ein', 'warning');
            return;
        }
        ...
        try {
            const response = await fetch('../php/chat_api.php', {
                ...
                body: `action=update_profile&username=${encodeURIComponent(username)}&profile_img=${encodeURIComponent(profileImg)}&custom_status=${status}`
            });
            const data = await response.json();
            if (data.success) {
                // Update Session / LocalStorage if needed
                const raw = localStorage.getItem('jeopardy_auth_session');
                if (raw) {
                    const sess = JSON.parse(raw);
                    sess.display_name = username;
                    localStorage.setItem('jeopardy_auth_session', JSON.stringify(sess));
                }

                this.showNotification('✅ Profil erfolgreich aktualisiert', 'success');
                this.renderWelcomeUser(); // Welcome Nachricht oben links aktualisieren
                // Header Display aktualisieren falls vorhanden
                const headerDisplay = document.getElementById('userDisplayDB');
                if (headerDisplay) headerDisplay.textContent = username;
            } else {
                this.showNotification('❌ Fehler: ' + data.msg, 'danger');
            }
        } catch (e) {
            this.showNotification('❌ Fehler beim Speichern', 'danger');
        }
    }

    showNotification(msg, type = 'info') {
        // Falls AnimationsController existiert, nutze ihn
        if (typeof AnimationsController !== 'undefined' && AnimationsController.showNotification) {
            AnimationsController.showNotification(msg, type, 3000);
        } else {
            // Einfacher Fallback
            alert(msg);
        }
    ...
    renderWelcomeUser() {
        ...
        if (username) {
            // First try to get the display name from the session or local storage
            let displayName = username;
            try {
                if (window.authManager && window.authManager.isLoggedIn()) {
                    displayName = window.authManager.currentUser?.display_name || window.authManager.currentUser?.username || username;
                } else {
                    const raw = localStorage.getItem('jeopardy_auth_session');
                    if (raw) {
                        const sess = JSON.parse(raw);
                        displayName = sess?.display_name || sess?.username || username;
                    }
                }
            } catch (_) { }

            titleEl.textContent = `👋 Willkommen zurück, ${displayName}!`;
        } else {
