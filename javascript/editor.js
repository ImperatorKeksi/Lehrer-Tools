...
class QuestionEditor {
    ...
    renderQuestions(searchTerm = '') {
        ...
        if (!term) {
            // NORMAL MODE: Show questions for current category
            ...
            title.textContent = `Fragen: ${category.name}`;
            counter.textContent = `${category.questions.length} / 20 Fragen`;
            ...
            // GLOBAL SEARCH MODE: Show matches from all categories
            title.textContent = `Suchergebnisse für "${searchTerm}"`;

            let globalMatches = [];
            this.categories.forEach(category => {
                category.questions.forEach((question, index) => {
                    const qText = (question.question || '').toLowerCase();
                    const aText = (question.answer || '').toLowerCase();
            ...
            counter.textContent = `${globalMatches.length} Treffer gefunden`;

            if (globalMatches.length === 0) {
                container.innerHTML = `
                    <div class="editor-placeholder">
                        <p>🚫 Keine Treffer für "${this.escapeHtml(searchTerm)}" gefunden.</p>
                    </div>
                `;
            } else {
                globalMatches.forEach(match => {
                    ...
                    questionEl.classList.add('search-match-priority');
                    container.appendChild(questionEl);
                });
            }
        }
    ...
    createQuestionElement(question, number, categoryName = '', categoryId = '') {
        const el = document.createElement('div');
        el.className = 'question-item';
        el.setAttribute('data-question-id', question.id);
        el.setAttribute('data-category-id', categoryId || this.currentCategoryId);

        const pointsValue = typeof question.points === 'number' ? question.points : 100;
        const categoryLabel = categoryName ? `<span class="question-category-tag">📦 ${this.escapeHtml(categoryName)}</span>` : '';
            ...
            <div class="question-form">
                <div class="form-group">
                    ...
                    <textarea 
                        ...
                        data-category-id="${categoryId || this.currentCategoryId}"
                ...
                <div class="form-group">
                    ...
                    <select data-field="points" data-question-id="${question.id}" data-category-id="${categoryId || this.currentCategoryId}">
        ...
        el.querySelector('.btn-duplicate').addEventListener('click', (e) => {
            e.stopPropagation();
            this.duplicateQuestion(question.id, categoryId || this.currentCategoryId);
        });
        el.querySelector('.btn-delete').addEventListener('click', (e) => {
            e.stopPropagation();
            this.deleteQuestion(question.id, categoryId || this.currentCategoryId);
        });
    ...
    updateQuestionField(input) {
        ...
        // Update counter based on context
        if (!document.getElementById('editorSearchInput').value.trim()) {
            document.getElementById('questionCounter').textContent = `${category.questions.length} / 20 Fragen`;
        }
    }

    deleteQuestion(questionId, categoryId = null) {
        const targetId = categoryId || this.currentCategoryId;
    ...
    duplicateQuestion(questionId, categoryId = null) {
        const targetId = categoryId || this.currentCategoryId;
        const category = this.categories.find(c => c.id === targetId);
        if (!category) return;
        ...
        const searchTerm = document.getElementById('editorSearchInput').value;
        this.renderQuestions(searchTerm);
        this.renderCategories(); // Update question count
        ...
        setTimeout(() => {
            ...
            if (duplicatedEl) {
                ...
                duplicatedEl.style.animation = 'highlightPulse 1.5s ease-in-out';
            }
        }, 120);
    ...
    searchQuestions(searchTerm) {
        ...
        this.categories.forEach(category => {
            category.questions.forEach(question => {
                if (question.question.toLowerCase().includes(term) || question.answer.toLowerCase().includes(term)) {
                    matchCount++;
                }
            });
        });
        ...
        if (matchCount > 0) {
            resultCount.textContent = `${matchCount} Treffer`;
